...
###  实现一个端口隐藏dome（文件port.sys）支持win7-win11,可过ark工具检查，本dome没有严禁测试可能不稳定仅测试，是否触发pg短时间我也没测出来，单独一个驱动使用工具加载即可使用工具查看端口,默认只隐藏pid为4的tcp&udp端口,请在虚拟机运行测试，我随时可能就删除测试程序，不要问报毒问题懂则懂不懂也不解释，一切责任概不负责。
